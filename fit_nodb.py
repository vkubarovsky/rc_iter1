"""Same plain fit with db_ET FROZEN AT 0 (VPK: db=4 is crazy, and it is what
buys R_ET=4 -- a big normalisation at t=0 killed off by a steep slope).
...
def expand(x):
    p=np.zeros(27); p[FREE]=x; p[12]=0.0; p[14]=0.0; return p
def pen(p):
    r=[]
    for x in (0.10,0.60):
        s=slopes(p,x)
...
for f in ("fitpar_amp2026s.npy","fitpar_prior_none.npy","fitpar_prior_halla.npy","fitpar_tied.npy","fitpar_amp2026_lx.npy"):
    try: q=np.array(np.load(f)); q[12]=0.0; q[14]=0.0; seeds.append((f,q))
...
print(f"\n=== PLAIN FIT with db_ET = 0 === {n} pts, 25 par: "
      f"chi2/ndf = {tot:.1f}/{n-25} = {tot/(n-25):.4f}   penalty {sum(x*x for x in pen(p)):.3f}")
