...
on two derived quantities.  Series~8 has 9 sets, series~B adds Hall A 2021.}
\label{tab:loo}
\begin{tabular}{lccrr}
\toprule
removed & blind, series 8 & blind, series B & $d/u|_{H_T}$ & $n/p$\\
\midrule
--- (reference) & --- & --- & $-0.311$ & $1.062$\\
CLAS6 $\pi^0$ & 2.89 & 3.52 & $-0.322$ & 0.973\\
CLAS6 $\eta$ & \textbf{218.9} & \textbf{2.81} & $-0.475$ & 1.220\\
Hall A neutron & 10.06 & \textbf{13.06} & $+0.401$ & \textbf{2.153}\\
Hall A 2016 & 6.18 & 5.90 & $-0.335$ & 1.039\\
Hall A 2011 & 2.66 & 1.93 & $-0.354$ & 1.094\\
Hall A 2021 & --- & 13.10 & $-0.238$ & 1.037\\
De Masi $\phi$ & 1.02 & 1.02 & $-0.277$ & 1.072\\
Zhao & 2.25 & 1.90 & $-0.237$ & 1.086\\
CLAS12 BSA & 2.88 & 2.59 & $-0.334$ & 1.044\\
eg1-dvcs & \textbf{40.16} & \textbf{41.48} & $-0.405$ & 0.648\\
\bottomrule
\end{tabular}
\end{table}
...
per point.  With it in the fit that collapses to 2.81.  Hall A 2021 reaches
...
$-0.311$ to $-0.475$ when dropped; it is no longer irreplaceable.

\paragraph{The reference fit is reproducible.}  Removing Hall A 2021 from
series~B reproduces the series~8 reference to four figures, from a different
...
\paragraph{$n/p$ is separated by four points.}  The statement needs care,
because two different things are true of it.

The neutron channel is $\langle X\rangle \propto X_u + 2X_d$ against the
proton's $2X_u + X_d$, so the $d$ block enters the neutron with twice the
weight it has in the proton.  Since $\sig{TT} = \sum|T_{01}|^2 -
\sum|U_{01}|^2$ and $U_{01}$ is built from $\bar E_T$, the ratio $n/p$ in
$\sig{TT}$ is essentially a measurement of $\bar E_T^d$ relative to
$\bar E_T^u$.

The fitted $t$-slopes of those two blocks differ by a factor eight,
$b_d = 5.37 \pm 0.36$ against $b_u = 0.68 \pm 0.05$, which invites the
suspicion that this is the same flat valley that forced $b_d = b_u$ in the
$H_T$ sector.  It is not.  Imposing the same tie on $\bar E_T$ costs
\emph{2662 units of $\chi^2$ for one parameter} --- $\chi^2/\mathrm{ndf}$ from
$1.39$ to $3.13$ --- and the damage is spread over the whole database, not
carried by the neutron: CLAS6 $\pi^0$ goes from $1.68$ to $3.04$ per point,
CLAS6 $\eta$ from $1.04$ to $5.05$, eg1 from $1.85$ to $12.38$.  For
comparison, the same tie in the $H_T$ sector costs two units, which is what a
flat valley actually looks like.  The steepness of $\bar E_T^d$ is therefore a
requirement of the data, not an artefact.  (That test was started from the
unconstrained solution and is an upper bound on the constrained minimum; given
a factor $2.25$ in $\chi^2$ the conclusion does not depend on how tight the
bound is.)

What the neutron supplies is not the steepness but the \emph{separation}.  In
the proton combination $2X_u + X_d$ the normalisation $N_d$ and the slope $b_d$
of the $d$ block are degenerate --- $d$ sits at half the weight of $u$ and can
trade one against the other.  The neutron combination breaks that degeneracy,
and it is the only thing in the database that does.  Removing the four neutron
points moves $n/p$ from $1.06$ to $2.15$, and the movement is traceable almost
entirely to $b(\bar E_T^d)$ relaxing from $5.32$ to $3.57$; changing that one
parameter alone already carries $n/p$ to $1.45$.

So $n/p$ is constrained, but by four points with uncertainties of 42 to
90\,\%, and it is the least robust number in this report.  During one day of
otherwise minor and justified changes --- correcting systematics, adding a
normalisation nuisance, repairing a parameter bound --- it moved through
$1.00$, $1.02$, $0.98$, $1.04$, $1.06$ while $\chi^2/\mathrm{ndf}$ changed in
the third decimal.  It should be quoted with that history attached, and a fifth
neutron point would be worth more than a thousand proton points.
...
\textsc{not in the fit} marks a blind prediction.  Each panel carries its own
kinematics and $\chi^2$; the model curve includes the fitted normalisation of
...
\caption{eg1-dvcs, polarised target: the $A_{LL}$ moments.  These are the only
observables in the database sensitive to the relative phase of $T_{01}$ and
$U_{01}$; without them it is unconstrained.}
\end{figure}
