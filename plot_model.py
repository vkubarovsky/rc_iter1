...
import os
NEW=np.load(os.environ.get("NEWPAR","fitpar_amp2026s.npy")); OLD=np.load("fitpar_amp2026_lx.npy")
TAG=os.environ.get("TAG","amp2026s")
...
for rows,ch,meson,fname in ((F.PI0,"pi0p",r"\pi^0","xsec_pi0"),(F.ETA,"etap",r"\eta","xsec_eta")):
    ...
    h=[plt.Line2D([],[],color='k',lw=2,label=r'$\sigma_U$'),
       ...
       plt.Line2D([],[],color='k',lw=2,ls='-',label='new fit'),
       plt.Line2D([],[],color='k',lw=1.3,ls='--',label='amp2026 (27 par, previous)'),
       plt.Line2D([],[],color='k',marker='s',mfc='none',ls='none',ms=8,label='new fit averaged over the bin')]
    ...
    fig.tight_layout(rect=[0,0.03,1,0.965]); fig.savefig(f"figures/{fname}_{TAG}.png",dpi=115); plt.close(fig)
...
for j in range(n,nr*nc): axs[j//nc][j%nc].axis('off')
fig.legend(handles=[plt.Line2D([],[],color=CN,lw=2,label='new fit'),
...
fig.tight_layout(rect=[0,0.02,1,0.955]); fig.savefig(f"figures/bsa_pi0_clas6_{TAG}.png",dpi=115); plt.close(fig)
...
fig.tight_layout(rect=[0,0.02,1,0.94]); fig.savefig(f"figures/bsa_clas12_eta_{TAG}.png",dpi=115); plt.close(fig)
...
fig.tight_layout(rect=[0,0.02,1,0.94]); fig.savefig(f"figures/eg1_targets_{TAG}.png",dpi=120); plt.close(fig)
...
fig.tight_layout(rect=[0,0,1,0.95]); fig.savefig(f"figures/model_{TAG}.png",dpi=120); plt.close(fig)
