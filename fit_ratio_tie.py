...
def run(tag, mode, sector=True, tie_bET=False, tie_bHT=False):
    ...
    if tie_bET: ties[14]=9        # b_d = b_u in the Ebar_T sector
    if tie_bHT: ties[5]=1         # and in H_T
...
run("Rstat_free","ratio_stat")
run("Rstat_bET","ratio_stat",tie_bET=True)
run("Rstat_bAll","ratio_stat",tie_bET=True,tie_bHT=True)
run("Rsyst_bET","ratio_syst",tie_bET=True)
run("abs_bET","absolute",tie_bET=True)
