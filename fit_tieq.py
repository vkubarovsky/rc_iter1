...
def run(tag, halla=False, bsa2x=False, phase0=False, tieQ=False):
    free=[i for i in range(32)]
    if tieQ:
        free.remove(27); free.remove(7)
    if phase0: free.remove(23)
    def expand(x):
        p=np.zeros(32); p[free]=x
        if phase0: p[23]=0.0
        if tieQ: p[27]=p[11]; p[7]=p[3]
        return p
    def resid(x):
...
run("tieQ",halla=True,phase0=True,tieQ=True)
